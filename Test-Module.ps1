...
.EXAMPLE
    ...
    Will test the module in .\BuildOutput\cliHelper.gemini\0.1.0\
.EXAMPLE
    .\Test-Module.ps1
    Will test the latest  module version in .\BuildOutput\cliHelper.gemini\
#>
param (
  ...
  [ValidateScript({
      ...
      $b_Path = [IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'cliHelper.gemini')
...
begin {
  $TestResults = $null
  $BuildOutput = [IO.DirectoryInfo]::New([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'cliHelper.gemini'))
  ...
  $BuildOutDir = Resolve-Path $([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'cliHelper.gemini', $version)) -ErrorAction Ignore | Get-Item -ErrorAction Ignore
  if (!$BuildOutDir.Exists) { throw [System.IO.DirectoryNotFoundException]::new($BuildOutDir) }
  $manifestFile = [IO.FileInfo]::New([IO.Path]::Combine($BuildOutDir.FullName, "cliHelper.gemini.psd1"))
  ...
  if (!(Test-Path -Path $([IO.Path]::Combine($PSScriptRoot, "cliHelper.gemini.psd1")) -PathType Leaf -ErrorAction Ignore)) { throw [System.IO.FileNotFoundException]::New("Module manifest file Was not Found in '$($BuildOutDir.FullName)'.") }
  $script:fnNames = [System.Collections.Generic.List[string]]::New(); $testFiles = [System.Collections.Generic.List[IO.FileInfo]]::New()
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'cliHelper.gemini.Integration.tests.ps1')))
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'cliHelper.gemini.Features.tests.ps1')))
  [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'cliHelper.gemini.Module.tests.ps1')))
}

process {
  Get-Module cliHelper.gemini | Remove-Module -Force
  Write-Host "[+] Checking test files ..." -ForegroundColor Green
  $missingTestFiles = $testFiles.Where({ !$_.Exists })
  if ($missingTestFiles.count -gt 0) { throw [System.IO.FileNotFoundException]::new($($missingTestFiles.BaseName -join ', ')) }
